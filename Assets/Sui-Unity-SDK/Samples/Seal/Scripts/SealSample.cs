...
namespace Sui.Seal
{
    public class SealSample : MonoBehaviour
    {
        ...
        public async void OnDecryptButtonClick()
        {
            ...
            var response = await _client.GetObjectAsync(new AccountAddress(objectId), new ObjectDataOptions() { ShowContent = true });
            ...
            byte[] decryptedBytes = await SealBridge.Instance.Decrypt(encryptedBytes, nonceBytes, objectId, _ephemeralAccount.PrivateKey.ToBase64(), EnokiZKLogin.GetSuiAddress());
